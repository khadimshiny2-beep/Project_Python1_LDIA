...
from statistiques import analyser_fichier_patients
...
def option_statistiques():
    ...
    stats = analyser_fichier_patients(
        etat["patients_valides"],
        nb_total_brut=len(etat["lignes_brutes"]),
        nb_doublons=etat["nb_doublons"],
        nb_rejetes=len(etat["patients_rejetes"]),
    )
    etat["stats"] = stats

    age_moyen   = stats.get("age_moyen", "N/A")
    poids_moyen = stats.get("poids_moyen", "N/A")
    age_aff   = f"{age_moyen:.2f} ans" if isinstance(age_moyen, (int, float)) else str(age_moyen)
    poids_aff = f"{poids_moyen:.2f} kg" if isinstance(poids_moyen, (int, float)) else str(poids_moyen)

    separateur(C.CYAN)
    print(f"  {C.BOLD}{C.BLANC}Patients valides        {C.RESET}: {C.VERT}{stats.get('nb_valides', 0)}{C.RESET}")
    print(f"  {C.BOLD}{C.BLANC}Moyenne des âges        {C.RESET}: {C.CYAN}{age_aff}{C.RESET}")
    print(f"  {C.BOLD}{C.BLANC}Moyenne des poids       {C.RESET}: {C.CYAN}{poids_aff}{C.RESET}")
    print(f"  {C.BOLD}{C.BLANC}Ville la plus fréquente {C.RESET}: {C.VIOLET}{stats.get('ville_top', 'N/A')} "
          f"({stats.get('ville_top_count', 0)} patients){C.RESET}")

    separateur(C.CYAN)
    print(f"  {C.BOLD}{C.BLANC}RÉPARTITION DES GROUPES SANGUINS{C.RESET}")
    separateur(C.GRIS)

    for groupe, nb in stats.get("groupes_sanguins", {}).items():
        barre = "█" * nb
...
def option_exporter():
    ...
    if not etat["stats"]:
        etat["stats"] = analyser_fichier_patients(etat["patients_valides"])
